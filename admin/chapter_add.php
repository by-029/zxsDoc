...
$stmt = $db->prepare("SELECT MAX(`order`) as max_order FROM chapters WHERE project_id = ? AND parent_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $order = intval($_POST['order'] ?? $next_order);
    
    if (empty($title)) {
        $error = '标题不能为空';
    } else {
        $slug = generateSlug($db, 'chapters', $project_id, $parent_id);
        ...
        try {
            if (empty($html_content) && !empty($content)) {
                ...
                $stmt = $db->prepare("INSERT INTO chapters (project_id, parent_id, title, slug, content, html_content, `order`) VALUES (?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([$project_id, $parent_id, $title, $slug, $content, $html_content, $order]);
                
            header('Location: chapter_list.php?project_id=' . $project_id);
                exit;
            } catch(PDOException $e) {
                error_log("Chapter creation failed: " . $e->getMessage());
                $error = '创建章节失败，请稍后重试';
        }
    }
}
...
<head>
    ...
    <title>新建章节 - <?php echo SITE_NAME; ?></title>
    ...
    <div class="admin-container">
        ...
        <div class="admin-content">
            <div class="page-header">
                <h2>新建章节 - <?php echo htmlspecialchars($project['name']); ?></h2>
                ...
                <form method="POST" id="chapterForm">
                    ...
                    <div class="form-group">
                        ...
                        <div class="order-input-wrapper">
                            <input type="number" name="order" value="<?php echo $next_order; ?>" min="0" step="1" class="order-input-field" id="orderInput">
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <div id="contentEditor">
                            <textarea name="content" style="display:none;"><?php echo htmlspecialchars($_POST['content'] ?? ''); ?></textarea>
                        </div>
                    </div>
                    
                    <div style="display: flex; gap: 10px;">
                        <button type="submit" class="btn btn-primary">保存章节</button>
                        <a href="project_edit.php?id=<?php echo $project_id; ?>" class="btn btn-secondary">取消</a>
                    </div>
                </form>
            </div>
        </div>
            ...
            var editor = editormd("contentEditor", {
                ...
                toolbarHandlers: {
                    ...
                    image: function() {
                        ...
                        var overlay = $("<div class=\"editormd-image-dialog-overlay\"></div>").css({
                            "position": "fixed",
                            "top": "0",
                            "left": "0",
                            "right": "0",
